...
"""
def func4(*kargs):
    l = filter(lambda x:isinstance(x,str),kargs)
    if l:
        ll=[len(x) for x in l]
        return l[ll.index(min(ll))]
    return None
'''
    for i in kargs:
        if isinstance(i,str):
...
def func5(name=None,**kwargs):
    l = ["%s:%s" % (k,v) for k,v in kwargs.items()]
    l.insert(0,name)
    return ",".join(l)
    
'''
    result = name + ','
    result += str(dict(kwargs.items()))[1:-1]
    return result
'''
...
'''
    ...
    if d:
        l=str(d)
        return "%s,%s" % (name,l[1:-1])
    return "%s" % name
'''
print func5('lilei')
print func5("lilei",years=4)
